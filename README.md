...
> 🎬 **KodakTV** 是一个开箱即用的、跨平台的影视聚合播放器。它基于 **Next.js 14** + **Tailwind&nbsp;CSS** + **TypeScript** 构建，支持多资源搜索、在线播放、收藏同步、播放记录、云端存储，让你可以随时随地畅享海量免费影视内容。
...
[![Deploy on Zeabur](https://zeabur.com/button.svg)](https://zeabur.com/templates/F0DG0F/deploy)
...
**⚠️ 重要提示**：部署完成后，需要在 Zeabur 中为 KodakTV 服务设置访问域名（Domain）才能在浏览器中访问。详见下方 [设置访问域名](#5-设置访问域名必须) 步骤。

模板发布后，我们会把一键部署按钮固定为你的 KodakTV 模板链接，后续用户即可一键部署正确环境。
...
services:
  kodaktv-core:
    ...
    environment:
      ...
      - KVROCKS_URL=redis://kodaktv-kvrocks:6666
    networks:
      - kodaktv-network
    depends_on:
      - kodaktv-kvrocks
  kodaktv-kvrocks:
    ...
    container_name: kodaktv-kvrocks
...
services:
  kodaktv-core:
    ...
    environment:
      ...
      - REDIS_URL=redis://kodaktv-redis:6379
    networks:
      - kodaktv-network
    depends_on:
      - kodaktv-redis
...
2. **添加 KodakTV 服务**
   ...
   - 点击 "Add Service" > "Docker Images"
   - 输入镜像名称：`ghcr.io/korean032/kodaktv:latest`
   - 配置端口：`3000` (HTTP)

3. **配置环境变量**

   在 KodakTV 服务的环境变量中添加：
...
KodakTV 支持标准的苹果 CMS V10 API 格式。

## 订阅

将完整的配置文件 base58 编码后提供 http 服务即为订阅链接，可在 KodakTV 后台/Helios 中使用。
...
| NEXT_PUBLIC_SITE_NAME               | 站点名称                 | 任意字符串               | KodakTV                                                                                                                    |
...
[MIT](LICENSE) © 2025 KodakTV & Contributors
...
[![Star History Chart](https://api.star-history.com/svg?repos=MoonTechLab/KodakTV&type=Date)](https://www.star-history.com/#MoonTechLab/KodakTV&Date)
