...
// ---- 常量 ----
const PLAY_RECORDS_KEY = 'kodaktv_play_records';
const FAVORITES_KEY = 'kodaktv_favorites';
const SEARCH_HISTORY_KEY = 'kodaktv_search_history';

// 缓存相关常量
const CACHE_PREFIX = 'kodaktv_cache_';
...
// 页面加载时清理过期缓存
if (typeof window !== 'undefined') {
  // 本地存储键迁移：将旧的 moontv_* 键迁移到新的 kodaktv_*
  try {
    const migrations: Array<{ oldKey: string; newKey: string }> = [
      { oldKey: 'moontv_play_records', newKey: 'kodaktv_play_records' },
      { oldKey: 'moontv_favorites', newKey: 'kodaktv_favorites' },
      { oldKey: 'moontv_search_history', newKey: 'kodaktv_search_history' },
      { oldKey: 'moontv_skip_configs', newKey: 'kodaktv_skip_configs' },
    ];
    ...
    // 迁移用户缓存（moontv_cache_* -> kodaktv_cache_*）
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key && key.startsWith('moontv_cache_')) {
        const suffix = key.substring('moontv_cache_'.length);
        const newKey = `${CACHE_PREFIX}${suffix}`; // 使用新前缀
        const hasNew = localStorage.getItem(newKey) !== null;
        if (!hasNew) {
...
export async function getSkipConfig(
  ...
  // localStorage 模式
  try {
    const raw = localStorage.getItem('kodaktv_skip_configs');
    if (!raw) return null;
...
export async function saveSkipConfig(
  ...
  try {
    ...
    configs[key] = config;
    localStorage.setItem('kodaktv_skip_configs', JSON.stringify(configs));
    window.dispatchEvent(
      new CustomEvent('skipConfigsUpdated', {
        detail: configs,
      })
